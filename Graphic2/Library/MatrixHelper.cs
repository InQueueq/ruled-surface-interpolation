...
namespace Graphic2.Library
{
    static class MatrixManager
    {
        ...
        public static Point3D OnAngleX(Point3D point, float angle) // Обертання навколо центральної точки по осі ОХ
        {
        ...
        public static Point3D OnAngleY(Point3D point, float angle) // Обертання навколо центральної точки по осі ОY
        {
            angle = angle * (float)Math.PI / 180;
            float[,] matrix = GetCleanMatrix();
        ...
        public static Point3D OnAngleZ(Point3D point, float angle) // Обертання навколо центральної точки по осі ОZ
        {
            angle = angle * (float)Math.PI / 180;
            float[,] matrix = GetCleanMatrix();
            matrix[0, 0] = (float)Math.Cos(angle);
        ...
        public static Point3D Isometric(Point3D point, float angleX, float angleY, float angleZ, Point3D mover = null)
        {

            point = OnAngleX(point, angleX);
            point = OnAngleY(point, angleY);
            point = OnAngleZ(point, angleZ);
